...
export function part1(fileContents: string) {
  const parsed: CoordsList = parseInput(fileContents).filter(([[x1, y1], [x2, y2]]) => x1 === x2 || y1 === y2);
...
function calculateCoordsMap(coordsMap: CoordsList): CoordsMap {
  ...
  return coordsList.reduce<CoordsMap>(
    (map: CoordsMap, coords: Coords) => {
...
function getLines([x1, y1]: Coords, [x2, y2]: Coords): Coords[] {
  ...
  return range(Math.max(xRange, yRange)).map((n) => [x1 + xDelta * n, y1 + yDelta * n]);
}
