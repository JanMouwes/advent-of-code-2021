...
export function fromGrid<T>(
  grid: Grid<T>,
  cost: Fn2<[T, Coord], [T, Coord], number> = constant(1)
): Graph<[T, Coord]> {
...
export function findPaths<T>(start: Node<T>): Map<Node<T>, Path<T>> {
  const queue: [[Node<T>, Edge<T>], number][] = second(start).map((edge) => [
    [start, edge],
    0,
  ]);
  ...
  const entries: [Node<T>, Path<T>][] = [...known.keys()].map((node) => [
    node,
    collapsePath(node, known),
  ]);
...
export function findPath<T>(
  ...
  heuristic: Fn2<Node<T>, Node<T>, number> = constant(1)
): Path<T> {
  type QueueEntry = { node: [Node<T>, Edge<T>]; total: number; heur: number };

  let queue: Heap<QueueEntry> = new Heap<QueueEntry>(
    ({ total: t1, heur: h1 }, { total: t2, heur: h2 }) => t1 + h1 - (t2 + h2)
  );
  second(start).map((edge) =>
    queue.insert({ node: [start, edge], total: 0, heur: 0 })
  );
...
function collapsePath<T>(
  node: Node<T>,
  known: Map<Node<T>, [Edge<T>, number]>
) {
  const path: Edge<T>[] = [];

  if (!known.has(node)) {
    throw new Error("node not in known");
  }
