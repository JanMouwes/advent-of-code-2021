...
export function part1(fileContents: string) {
  ...
  function findPaths(path: string[], graph: Graph<string>): string[][] {
    ...
    return [...edges]
      .map((edge) => {
        if (smallCaves.includes(edge) && currentPath?.includes(edge)) {
          return [];
        }
    ...
    return [...edges]
      .map((edge) => {
        ...
        function countSmallCaves() {
          const cnts = count(
            path.filter((x) => smallCaves.includes(x)),
            id
          );
