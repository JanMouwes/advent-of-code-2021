...
export function part1(fileContents: string) {
  ...
  const bottomRight: Tuple<number, number> = (([x, y]) => [x - 1, y - 1])(
    size(grid)
  );
...
function showPath<T>(grid: Grid<T>, path: Path<Tuple<any, Coord>>) {
  ...
  const newGrid = map(strGrid, (i, c) =>
    coords.find((c2) => equals(c, c2)) ? i.green : i
  );
  ...
  return grids
    .map(([head, ...gridRow]) => {
      return gridRow.reduce(
        (current, grid) => grid.map((row, i) => [...current[i], ...row]),
        [...head]
      );
    })
    .flat();
